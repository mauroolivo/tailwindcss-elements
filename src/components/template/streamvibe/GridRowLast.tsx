...
export default function GridRowLast({ images }: GridRowProps) {
  ...
  return (
    <div className="inline-flex h-24 items-start justify-start gap-2.5 self-stretch">
      {imgs.map((src, i) => (
        <Image
          key={i}
          className="flex-1 self-stretch rounded-xl object-cover"
          src={getPath(src)}
          alt={`grid-${i + 1}`}
          width={151}
          height={100}
